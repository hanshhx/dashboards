...
export default function IpAnalysisPage() {
  ...
  const cls =
    'px-3 py-2 rounded-lg bg-slate-100 dark:bg-white/10 text-slate-900 dark:text-slate-100 placeholder:text-slate-400 border border-slate-300 dark:border-white/15 focus:border-violet-500 outline-none text-sm';
      ...
      {p && (
        <>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 mt-4">
            ...
            <Card title="시간선" sub="시간대별 활동"><MiniBars data={p.timeline} color="#7c5cff" maxH={280} /></Card>
          </div>
        </>
      )}
    </Shell>
