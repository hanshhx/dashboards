...
- 기초 문제를 처음 24개에서 104개(4지선다·상세 해설)로 보강 완료. 24개 항목 모두 문제를 보유하며, 문항 수·4지선다·정답 위치·상세 해설·항목 연결의 무결성을 스크립트로 검증.
- 심화 학습 항목 32개(이론)와 심화 문제 100개(5지선다·상세 해설)를 작성·검증 완료. 실제 CVE는 NVD·Cisco 출처로 확인함(Cisco RV320/RV325 = 인증 없는 설정 정보 유출 CVE-2019-1653, D-Link DSL-2750B = 인증 없는 원격 명령 주입 CVE-2016-20017·CVSS 9.8). 진단으로 받은 공격 분류 24종을 기초·심화에 걸쳐 모두 반영.
- 입문 문제를 100개(3지선다·상세 해설)로 보강 완료. 이로써 학습 콘텐츠 3단계가 모두 채워짐 — 학습 항목 75개, 문제 309개(입문 18항목·100문제, 기초 25항목·109문제, 심화 32항목·100문제). 진단으로 받은 공격 분류 24종과 희귀 프로토콜·허니팟 생애주기 이벤트(데이터 long tail)까지 한 항목으로 묶어 빠짐없이 반영. 단계 시험은 무작위 20문항 중 80% 이상 통과 시 다음 단계 해금. 보기 수는 입문 3·기초 4·심화 5지선다로 통일.
...
### 2026-06-16 — 학습 화면 UX 개선(복습·진도·용어집·사례 연구·삽화)
- 복습·오답노트 페이지(/learn/review) 추가: 퀴즈에서 틀린 문제를 모아 다시 풀 수 있고(맞히면 목록에서 빠짐), 작성한 오답노트를 한곳에서 보고 수정(자동 저장). 연습·시험 양쪽에서 정답 여부를 기록하도록 퀴즈 채점을 연결.
- 학습 항목 이전/다음 이동 버튼 추가. 잠긴 다음 단계로는 '퀴즈 통과 후 열림'으로 비활성 표시. 이동 시 주소(?item=)도 함께 갱신.
- 전체 학습 진도율 막대와 완주 표시 추가(완료 항목 수 / 전체, 백분율). 모두 완료하면 완주 배지 표시.
- 용어집 페이지(/learn/glossary) 추가: 모든 학습 항목의 핵심 용어 159개를 모아 용어·뜻으로 검색하고, 출처를 눌러 해당 설명으로 이동. 같은 용어는 출처를 합쳐 표시.
- 본문 검색 강화: 항목명·태그·목표·요약·용어에 더해 본문 단락까지 검색 대상에 포함.
- 화면 문구의 잔여 그림문자 정리: 잠금 안내의 자물쇠와 통과 표시의 체크 그림문자를 디자인 시스템 아이콘으로 교체. 변경마다 빌드·문체 점검 통과.
- 사례 연구 워크스루 페이지(/learn/case) 추가: 허니팟 공격을 정찰 → 접속 → 자격증명 공격 → 침투 후 행동 → 종합 판단의 5단계로 따라가는 인터랙티브. 각 단계마다 공격자 행동·센서 로그·분석가 화면·핵심을 함께 보여 줌. 특정 실제 사건이 아니라 데이터에서 반복 관찰되는 패턴을 재구성한 학습용 사례이며, 예시 IP는 문서 전용 주소(203.0.113.x)를 사용.
- 학습 항목 삽화 14종 선별 보강: 입문 7(3단계 연결·이벤트 4유형·네트워크 계층·TCP와 UDP·DNS·권한 등급·ET 분류), 기초 2(SYN 스텔스 스캔·프로토콜 도넛), 심화 5(명령 주입·DoS 증폭·시도 대 성공·MITRE 단계·위협 헌팅 흐름). 텍스트로 충분한 CVE·표 항목은 제외. 그림은 직접 그린 다이어그램으로 라이트·다크 모두 자연스럽게 보이며, 14종 모두 렌더 좌표를 측정해 화면 밖 넘침·좌표 오류가 없음을 확인.

### 2026-06-16 — 백엔드: 학습 진도·오답노트 서버 동기화
- 로그인 사용자의 학습 진도(클리어 단계·완료 항목·오답노트·틀린 문제)를 계정에 저장해 기기 간 동기화. 지금까지는 브라우저(localStorage)에만 있어 기기를 바꾸면 사라졌음.
- 저장 위치: 회원 DB(dashboard_app)에 learn_progress 테이블 신설(사용자 1명당 JSON 1행). 보안 로그 데이터(suricata_db)는 일절 건드리지 않고, 기존 읽기/쓰기 회원 DB·계정을 그대로 재사용.
- 백엔드: LearnController(GET·PUT /api/learn/progress, 로그인 필요·일반 이상) + LearnProgressRepository(JdbcTemplate upsert). 문제 콘텐츠는 정적이라 DB에 두지 않고 진도만 보관.
- 프론트: 학습·퀴즈·복습 진입 시 서버 진도를 받아 로컬과 병합(배열은 합집합·메모는 로컬 우선), 변경 시 0.8초 디바운스로 서버에 저장. 로그인 안 했거나 백엔드 미적용이면 조용히 로컬만 사용(기존 동작 유지).
- 적용 위치(배포 중 확인): 회원 DB(dashboard_app)는 vm121 로컬(localhost:5432)에 있고, 데이터 DB(suricata_db)는 vm120에 있음. 따라서 learn_progress 테이블은 vm121의 dashboard_app에 생성하고, vm121에서 git pull·재빌드(./gradlew clean build)·sentinel-api 재시작으로 적용. 프론트는 백엔드 적용 전이라도 안전하게 동작(404면 로컬만 사용).

### 2026-06-16 — 구조 점검: 데이터 DB와 회원 DB 분리 유지(결정)
- 현재 구조를 재확인함 — 데이터 DB(suricata_db, 보안 로그 35만 건)는 vm120, 회원·진도 DB(dashboard_app, app_user·learn_progress)는 vm121 로컬(APP_DB_HOST=localhost). 백엔드는 두 데이터소스를 함께 사용(suricata_db=읽기 전용, dashboard_app=읽기/쓰기).
- 두 DB를 vm120으로 통합할지 검토했으나 분리 유지로 결정. 이유 — (1) 읽기 전용 데이터(vm120)를 쓰기 테이블과 같은 서버에 두지 않아 보호, (2) 로그인·진도를 vm121 로컬에서 처리해 빠르고 vm120 회선 장애에도 인증이 계속됨, (3) 분석 쿼리(vm120)와 인증(vm121)의 부하 분리, (4) 두 DB 간 조인이 없어 한곳에 모을 이유가 없음.
- 통합은 dashboard_app 덤프·복원, .env의 APP_DB_HOST 변경, vm120 원격 접속 허용(pg_hba)·계정 비밀번호 재설정·재테스트가 필요해 과거 502/500 장애와 같은 부류의 리스크가 있고 이득(백업 일원화 정도)이 적어 보류. 통합이 필요하면 기능 롤아웃 이후 별도 점검창에서 진행.

### 2026-06-16 — 3중 감사 후 개선(사실 정확도·동기화·UX·접근성·보안)
- 사실 정확도: CVE-2019-1653 영향 펌웨어를 1.4.2.15·1.4.2.17로 정정(웹 재검증; 초기 패치 불완전로 완전 수정은 1.4.2.22 — 기존 권고 유지). CVE-2016-20017 "1.05 미만"은 NVD 원문(devices before 1.05)대로 정확함을 재확인(수정 없음). 삽화에 하드코딩돼 있던 "약 54%"와 STUN 5349 표기를 데이터 갱신에도 안 틀어지게 정리.
- 진도 동기화: 합집합 병합이 삭제·초기화를 되살리던 버그를 "최초 1회만 합집합, 이후 서버 기준"으로 수정(완료 해제·초기화·오답 해결이 기기 간 전파됨). 탭 종료 시 마지막 변경 유실 방지(pagehide/visibilitychange 플러시 + fetch keepalive). 서버 JSON 안전 정제. pull 도중 변경 경합 가드.
- 품질·UX: 남아 있던 응원체(수고했어요!·도전하세요·열어 보세요·저장돼요·아쉬워요/정답!)를 중립 표현으로 정리. 모바일에서 학습 탭이 넘치던 것을 줄바꿈 처리. 잠긴 항목 안내를 "어느 단계 퀴즈를 통과해야 하는지"로 구체화 + 퀴즈 링크. 딥링크 진입 시 깜빡임·하이드레이션 불일치 방지(로딩 표시 후 본문). 요약 모드에서 즉시 완료되던 자동완료를 전체 모드에서만 동작하도록 제한.
- 접근성: 삽화 SVG에 설명(aria-label) 부여, 토글·탭에 선택 상태(aria-pressed·aria-current) 표시.
- 보안: (백엔드) 진도 저장 본문을 64KB·JSON 객체로 제한, 손상값 조회 시 500 대신 빈 객체 복구, 데이터·회원 DB에 statement_timeout 15초(무거운 쿼리 강제 종료), 비밀번호 해시 강도 BCrypt 12로 상향. (프록시) 요청 본문 512KB 상한.
- 검증: 변경마다 next build 통과 + 보강한 정규식으로 어조 0건 재확인. 백엔드는 로컬 Gradle이 환경 제약(루프백)으로 안 돌아 vm121 재빌드가 최종 컴파일 검증.

### 이후 진행 예정
- 학습 콘텐츠(이론 항목·문제)는 입문·기초·심화 세 단계 모두 완료. 남은 것은 아래 화면 개선과 백엔드 기능.
- 감사 후속(사용자 결정·실행 필요): 시그니처별 건수(count) DB 실측 대조, 인증 레이트리밋(클라이언트 IP 전달 필요), JWT 폐기/버전, 로그 내보내기 스트리밍, 가입 정책, DB 정기 백업(suricata_db·dashboard_app), 자동화 테스트·모니터링.
- 백엔드 기능: 진도·오답노트 서버 동기화 구현 완료(적용 런북 제공). 남은 것은 학습 분석 대시보드·실제 데이터 기반 동적 문제·서버 채점.
- 학습 화면 개선: 오답노트 모아보기·다음/이전 이동·전체 진도율·용어집·사례 연구·삽화 보강까지 완료. 프론트엔드 학습 기능은 일단락.

## 16. 버전별 상세 변경 이력 (v0.1.0 ~ v0.8.2)

GitHub 릴리즈 노트보다 더 자세히 정리한 버전별 변경 내역입니다. (저장소: hanshhx/dashboards, Jinseok-Yeom/2026-packet-board)

### v0.1.0 — 초기 구축
- 프로젝트 최초 버전. Suricata eve.json 기반 보안 로그를 PostgreSQL에 적재하고 보여 주는 웹 대시보드의 뼈대를 세움.
- 백엔드: Spring Boot 3.3.5 · Java 21, MyBatis로 PostgreSQL 로그 테이블(suricata_logs) 조회, REST API 제공.
- 프론트엔드: Next.js(App Router) · TypeScript · Tailwind CSS. 서버 route handler가 /api/* 를 백엔드로 프록시.
- 화면: 개요(총 이벤트·시간대별 추이)와 이벤트(로그) 목록.
- 로그 컬럼: id, timestamp, event_type, src_ip, src_port, dest_ip, dest_port, proto, payload_json. signature·severity·category는 payload_json(jsonb)에서 추출.

### v0.2.0 — API 키 인증
- 프록시와 백엔드 사이 통신 보호. 모든 /api/** 요청에 X-API-Key 헤더 검증(ApiKeyFilter), /api/health 만 예외(상태 점검용).
- 키는 서버 환경변수(API_KEY)로만 주입되어 브라우저에 노출되지 않음. 프론트 프록시가 서버 측에서 헤더를 붙임.
- 키 비교는 상수 시간 비교로 처리(타이밍 공격 방지).

### v0.2.1 — text timestamp 컬럼 대응
- 일부 데이터에서 시각이 날짜형이 아니라 ISO-8601 문자열(text)로 저장된 경우를 대응. 기간 필터·정렬에서 timestamp를 timestamptz로 캐스팅하도록 쿼리 보정.
- 문자열 정렬로 최신 로그가 뒤섞여 보이던 현상 해결.

### v0.2.2 — 대시보드 UI 개선
- 개요 화면 구성·차트·레이아웃 정돈.
- 카드·그래프 배치와 여백·정렬을 통일해 주요 지표(총 이벤트·경보·추이)를 상단에 모아 한눈에 보이게 개선.

### v0.3.0 — 로그 탐색 개편
- "로그 탐색" 화면 개편. 목록에서는 원문(payload)을 숨기고 시그니처 위주로 표시(가독성·보안).
- 단건 원문 조회 API: GET /api/events/{id}/payload (id가 text여도 동작).
- 요약: 시각별 로그 개수 히스토그램, 상위 출발지 IP 순위.
- 전체 보기 토글: 시각·유형·출발지·목적지·프로토콜·시그니처 표를 50건 단위로 표시.
- payload 모달에서 JSON·CSV 다운로드.

### v0.4.0 — 인증·권한 도입
- Spring Security + JWT(HS256) + BCrypt 기반 로그인·회원가입.
- 회원·인증 전용 DB(dashboard_app, 소유자 dashboard_app_user)에 app_user 테이블 — 데이터 DB(suricata_db)와 분리.
- 3등급 권한: 일반(GENERAL) / 관계자(STAFF) / 관리자(ADMIN). 화면·API 차등 접근(SecurityConfig).
- 프론트: 로그인 가드(Shell), 토큰 저장(localStorage), 로그인·회원가입 페이지. 프록시가 Authorization 헤더 전달.
- 최초 관리자: 가입 후 DB에서 role을 ADMIN으로 승급.

### v0.4.1 — payload 단건 조회 수정
- id가 text 타입인 환경에서 단건 원문(payload) 조회가 되지 않던 문제 수정(id를 text로 비교하도록 보정).

### v0.5.0 — 경보 정렬·분류/포트·기간·내보내기
- 경보 위험도순 정렬 + 심각도(높음 / 중간 / 낮음 / 정보) 필터.
- 공격 분류(category) 통계, 대상 포트(Top Ports) 통계.
- 기간(from / to) 필터(타임스탬프 캐스팅 처리).
- 필터 결과 일괄 내보내기(CSV, 원문 제외).

### v0.6.0 — 분석 드릴다운·프로필·감사로그
- IP 분석·시그니처 분석 드릴다운(관계자 이상): 한 주소 / 규칙의 활동을 한 화면에 모아 봄.
- 프로필·비밀번호 변경(전체 등급).
- 감사 로그(관리자): audit_log 테이블 자동 생성, 로그인·등급 변경·원문 열람 등 민감 행위를 기록.

### v0.7.0 — MITRE ATT&CK 매트릭스
- 공격 분류(category)를 MITRE ATT&CK 전술에 매핑한 히트맵 화면 추가.
- "어느 단계(정찰·침투·실행 등)의 공격이 많은가"를 한눈에 파악.

### v0.7.1 — 분석 드릴다운·UI 정리·백엔드 보안 가드
- 시그니처 ↔ IP 교차 드릴다운(클릭으로 상호 이동), 위험도 색구분 막대.
- UI 정리(AI 생성 티 제거): 보라 그라데이션을 브랜드 단색(#2563eb)으로, 제품명·색 토큰·용어를 통일·현지화, 브라우저 탭 제목·감사로그 배지 색 정합.
- 백엔드: 관리자 자기-강등 / 삭제 가드, 500 에러 메시지 마스킹(내부 정보 노출 방지).

### v0.8.0 — 학습 콘텐츠 3단계
- 학습 메뉴(/learn) 도입. 입문·기초·심화 3단계, 총 75개 학습 항목.
- 309문제: 입문 3지선다 · 기초 4지선다 · 심화 5지선다, 문항마다 상세 해설.
- 게임형 단계 잠금: 단계 시험(무작위 20문항)에서 80% 이상 통과 시 다음 단계 해금(localStorage로 진행 상태 관리).
- 연습 / 시험 모드 분리. 콘텐츠 파일 모듈화(공용 타입 + 입문 / 기초 / 심화 분리).
- 데이터 분석 결과 반영: 이벤트 유형(허니팟 약 54% 등), 24개 공격 분류, 369종 시그니처, 희귀 프로토콜·허니팟 생애주기 이벤트까지 항목화. 실제 CVE는 NVD·Cisco 출처로 검증.

### v0.8.1 — 학습 UX 일습 + 진도 동기화
- 복습·오답노트(/learn/review): 틀린 문제만 다시 풀기 + 문제별 메모(자동 저장).
- 용어집(/learn/glossary): 학습 항목에서 모은 핵심 용어 159개, 용어·뜻 검색, 출처로 이동.
- 사례 연구 워크스루(/learn/case): 허니팟 공격을 정찰 → 접속 → 자격증명 → 침투 후 → 종합의 5단계로 따라가는 인터랙티브.
- 전체 진도율·완주 표시, 이전 / 다음 항목 이동, 본문(단락)까지 검색 강화, 직접 그린 삽화 14종.
- 진도·오답노트 서버 동기화: GET / PUT /api/learn/progress + 회원 DB(dashboard_app)에 learn_progress 테이블(사용자당 JSON 1행). 변경 시 서버에 저장(디바운스), 진입 시 서버에서 병합. 로그인 안 했거나 백엔드 없으면 localStorage만 사용(폴백).

### v0.8.2 — 품질·보안 개선 (3중 감사 반영)
- 프론트 / 백엔드·보안 / 데이터·콘텐츠 세 방향 독립 감사 후 발견 사항을 정리.
- 사실 정확도: CVE-2019-1653(Cisco RV320) 영향 펌웨어 정정(취약 1.4.2.15 · 1.4.2.17, 완전 수정 1.4.2.22 — 웹 재검증), CVE-2016-20017(D-Link) "1.05 미만"은 NVD 원문대로 정확함 재확인, 삽화에 하드코딩된 "약 54%" 수치 제거(데이터 갱신 대비).
- 진도 동기화 정합성: 병합 정책을 "최초 1회만 합집합, 이후 서버 기준"으로 변경(완료 해제·초기화·오답 해결이 기기 간 전파). 탭 종료 시 마지막 변경 유실 방지(pagehide / visibilitychange 플러시 + fetch keepalive). 서버 응답 정제(형식 무검증 방지). pull 도중 변경 경합 가드.
- 품질·UX: 남은 응원체(수고했어요! · 도전하세요 · 열어 보세요 · 저장돼요 · 아쉬워요 / 정답!)를 중립 표현으로 정리. 모바일에서 학습 탭이 넘치던 것을 줄바꿈 처리. 잠긴 항목 안내를 "어느 단계 퀴즈를 통과해야 하는지"로 구체화 + 퀴즈 링크. 딥링크 진입 시 깜빡임·하이드레이션 불일치 방지. 요약 모드에서 즉시 완료되던 자동완료를 전체 모드에서만 동작하도록 제한.
- 접근성: 삽화 SVG에 설명(aria-label), 토글·탭에 선택 상태(aria-pressed · aria-current) 표시.
- 보안: 진도 저장 본문 64KB · JSON 객체 검증, 손상값 조회 시 500 대신 빈 객체로 복구, 데이터·회원 DB에 statement_timeout 15초(무거운 쿼리 강제 종료), 비밀번호 해시 강도 BCrypt 12로 상향, 프록시 요청 본문 512KB 상한.

### 릴리즈·버전 관리 메모
- v0.1.0 ~ v0.8.2 태그를 hanshhx/dashboards와 Jinseok-Yeom/2026-packet-board 양쪽에 푸시하고 릴리즈를 14개씩 발행.
- Jinseok-Yeom는 기존 v0.1.0-beta(Pre-release)를 보존하고, main은 팀 README를 유지한 채 코드만 최신(v0.8.2)으로 갱신.
- 릴리즈 노트는 평서체(음 / 임)로 통일하고, 본문 상단의 큰 제목을 제거해 가독성 개선.
